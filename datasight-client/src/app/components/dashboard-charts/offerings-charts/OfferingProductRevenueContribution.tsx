...
export default function OfferingProductRevenuetContributionChart({ data }) {
    // Prepare the data in a format suitable for stacked bar chart
    const formattedData = data.reduce((acc, { offering_id, product_id, product_contribution_to_offering_revenue }) => {
        const existingOffering = acc.find(item => item.offering_id === offering_id);
        if (existingOffering) {
            existingOffering[product_id] = product_contribution_to_offering_revenue;
        } else {
            acc.push({
                offering_id, [product_id]: product_contribution_to_offering_revenue });
        }
        return acc;
    }, []);
