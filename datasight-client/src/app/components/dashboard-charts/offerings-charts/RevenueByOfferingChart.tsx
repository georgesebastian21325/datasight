...
interface RevenueByOfferingChartProps {
    data: DataItem[];
}
...
const RevenueByOfferingChart: React.FC<RevenueByOfferingChartProps> = ({ data }) => {
    ...
    const totalRevenue = sortedData.reduce(
        (acc, entry) => acc + (entry.total_offering_revenue || 0),
        0
    );
                    ...
                    <ChartTooltip
                        content={({ active, payload }) => {
                            if (active && payload && payload.length) {
                                const { offering_id, total_offering_revenue } =
                                    payload[0].payload;
                                const percentage = calculatePercentage(
                                    total_offering_revenue,
                                    totalRevenue
                                );
                                return (
                                    <div
                                    ...
                                    >
                                        ...
                                        <p>
                                            Revenue: ₱
                                            {total_offering_revenue.toLocaleString(
                                                "en-US",
                                                { minimumFractionDigits: 2 }
                                            )}
                                        </p>
