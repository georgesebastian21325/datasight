...
export default function RiskOptimizationTable() {
  ...
  useEffect(() => {
    async function fetchRiskOptimizationTable() {
      try {
        ...
        setUniqueValues({
          service_id: Array.from(new Set(formattedData.map(item => item.service_id))),
  ...
  const handleFilterChange = (key, value) => {
    ...
    const newFilteredData = data.filter(item =>
      (newFilters.service_id === '' || item.service_id === newFilters.service_id) &&
  ...
  return (
    <Card className="w-full mt-12">
      <CardHeader>
        <CardTitle className="text-2xl font-bold mb-3">Risk Optimization Table</CardTitle>
        <div className="flex space-x-4 mt-4">
          <Select onValueChange={(value) => handleFilterChange('service_id', value)}>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>Distributed Utilization %</TableHead>
            ...
            <TableBody>
              {filteredData.map((item, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell>{item.predicted_usage_percentage}%</TableCell>
                  <TableCell>
                    <span className={getStatusColor(item.current_status)}> {item.current_status} </span>
                  </TableCell>
                  <TableCell>{item.distributed_utilization_percentage}%</TableCell>
                  <TableCell>
                    <span className={getStatusColor(item.optimized_status)}> {item.optimized_status} </span>
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
