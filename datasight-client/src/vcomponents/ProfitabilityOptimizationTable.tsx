...
import { Card, CardContent, CardHeader, CardTitle } from "@/vcomponents/risk-optimization-table-components/card"
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/vcomponents/risk-optimization-table-components/select"
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/vcomponents/risk-optimization-table-components/table"
...
export default function ProfitabilityOptimizationTable() {
 ...
 const handleFilterChange = (key, value) => {
  ...
  const newFilteredData = data.filter(item =>
   (newFilters.resource_id === '' || item.resource_id === newFilters.resource_id) &&
   (newFilters.period === '' || item.period === newFilters.period) &&
   (newFilters.new_service_id === '' || item.new_service_id === newFilters.new_service_id)
  )
 ...
 return (
  <Card className="w-full mt-12">
   <CardHeader>
    <CardTitle className="text-2xl font-bold mb-3">Profitability Optimization Table</CardTitle>
    <div className="flex space-x-4 mt-4">
     <Select onValueChange={(value) => handleFilterChange('resource_id', value)}>
     ...
     <Table className="table-fixed w-full border-collapse">
      <TableHeader>
       <TableRow>
        ...
        <TableHead>Resource Optimized Usage</TableHead>
        <TableHead>New Service ID</TableHead>
        <TableHead>Predicted Demand %</TableHead>
        <TableHead>Deviation from Ideal Usage</TableHead>
        <TableHead>Predicted Usage Risk Status</TableHead>
        <TableHead>Resource Optimized Usage Risk Status</TableHead>
        <TableHead>Predicted Usage Finance Status</TableHead>
        <TableHead>Resource Optimized Usage Finance Status</TableHead>
       </TableRow>
      </TableHeader>
      <TableBody>
       {filteredData.map((item, index) => (
        <TableRow key={index}>
         ...
         <TableCell>{item.predicted_usage}</TableCell>
         <TableCell>{item.ideal_usage}</TableCell>
         <TableCell>{item.resource_optimized_usage}</TableCell>
         <TableCell>{item.new_service_id}</TableCell>
         <TableCell>{item.predicted_demand_percentage}%</TableCell>
         <TableCell>{item.deviation_from_ideal_usage}%</TableCell>
         <TableCell>
         ...
         <TableCell>
          <span className={getStatusColor(item.resource_optimized_usage_finance_status)}> {item.resource_optimized_usage_finance_status} </span>
         </TableCell>
        </TableRow>
       ))}
      </TableBody>
